...
const CinemaRevenue = () => {
  const { data: CinemaSalesStatsData, isLoading: CinemaSalesStatsLoading, isError: CinemaSalesStatsError } = useGetCinemaSalesStatsQuery();
  const { data: movie } = useGetMoviesStatsQuery();

  // State để quản lý trạng thái "Xem tất cả" cho phim và rạp
      ...
      <div className="mt-6 rounded-sm font-sans flex justify-between">
        ...
        <div className="w-1/2 p-3 mr-2 bg-gray-100 rounded">
          <div className="flex justify-between items-center mb-4">
            ...
            >
              {showAllMovies ? "Ẩn bớt" : "Xem tất cả"}
            </button>
          </div>
          <div className="overflow-hidden ">
            <table className="w-full table-auto bg-white">
              ...
              <tbody>
                {(showAllMovies ? movie?.moviesStats : movie?.moviesStats.slice(0, 5)).map((movieItem, index) => (
                  <tr key={index} className="border-t">
                    <td className="px-2 py-2 text-gray-600 max-w-[240px] flex-1 truncate">
                      {movieItem.name}
                    </td>
                    <td className="px-2 py-2 text-right text-gray-600">
                      {movieItem.totalTickets}
                    </td>
                    <td className="px-2 py-2 text-right text-gray-600">
                      {movieItem.totalRevenue.toLocaleString()} VNĐ
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="w-1/2 p-3 ml-2 bg-gray-100 rounded">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              onClick={() => setShowAllCinemas((prev) => !prev)}
              className="text-blue-500 hover:underline"
            >
              {showAllCinemas ? "Ẩn bớt" : "Xem tất cả"}
            </button>
          </div>
          <div className="overflow-hidden ">
            <table className="w-full table-auto bg-white">
              ...
              <tbody>
                {(showAllCinemas ? CinemaSalesStatsData : CinemaSalesStatsData?.slice(0, 5)).map((cinema, index) => (
                  <tr key={index} className="border-t">
                    <td className="px-2 py-2 text-gray-600 max-w-[240px] flex-1 truncate">
                      {cinema._id || "Unknown"}
                    </td>
                    ...
                    <td className="px-2 py-2 text-right text-gray-600">
                      {cinema.totalRevenue.toLocaleString()} VNĐ
                    </td>
                  </tr>
                ))}
              </tbody>
